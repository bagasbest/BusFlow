...
**Requirement:** "kalo udah pass bus stop tapi belum ke detect (warna passed bus stop yg sudah ke detect akan berubah jadi warna hijau) tapi titiknya sudah lewat itu harus automatically berubah jadi hijau"
...
**Requirement:** "di akhir video pas udah sampai bus stop akhir lalu muncul tulisan "You have reached the final stop" itu tulisannya harusnya hanya muncul sekali saja, tapi itu kebawa sampai ke Activity2 yg lain"
...
**Requirement:** "Tapi pas sya mau lanjut ke trip trakhir pas pencet Start Route itu dia nge crash.. nah pas di rerun appnya trus fetch data from cache data, tripnya sudah hilang.. itu di fix tuk crashnya sama kasi logic mungkin "unfinish trip atau trip yg belum di pencet END Trip ga bole di remove dlu"
...
**Requirement:** "mungkin bisa di buat lebih detail setiap pass bus stop itu di log semua data dari upcoming stop, current stop, sama data2 tuk perhitungan ETA, speed, lat, lon, scheduleStatusText, etc"
...
**Requirement:** "lalu di awal juga di log data yg masuk ke tiap activity (ScheduleActivity, MapActivity, RepActivity, BreakActivity)"
...
**Requirement:** "Lalu di log juga bus lainnya yg ke detect sama bus kita (PanelDetail) dan di log juga kalau bus lain sudah hilang ato sudah tdk beroperasi, tujuan dari bus lain ke mana jg"
...
**Requirement:** "data2 tuk perhitungan ETA, speed, lat, lon, scheduleStatusText, etc (kalau tidak salah ada di ScheduleStatusManager.kt)"
...
**Requirement:** "mungkin bisa di sederhanakan jangan tiap detik, tiap 5 detik ato kalau ada ide lain bole mas"
...
**Requirement:** "kalo ga slh si Current Time itu masih diambil dari Start Time si schedule, harusnya itu diambil dari current time tablet"

**Status:** ⚠️ **REVERTED TO ORIGINAL (INTENTIONAL)**

**Reason:** 
- Ketika diubah ke tablet current time, terjadi masalah **1200+ menit** dan **200++ menit**
- User meminta revert: "kembalikan ke sebelumnya"
- Sudah di-revert ke versi original yang menggunakan schedule start time
...
**Note:** ⚠️ **This is intentional revert to prevent 1200+ minutes issue. User explicitly requested this.**

---

## ⚠️ Requirement 6: ETA Calculation Accuracy

**Requirement:** "(fix nanti setelah log) untuk ETA nya itu sepertinya masih belum sempurna, tapi mungkin itu di fix nanti sehabis ada log supaya lebih jelas dan bsa di compare sama current time"

**Status:** ⚠️ **PENDING (AS REQUESTED)**

**Note:** User explicitly said "fix nanti setelah log", so this is intentionally pending until after logging is verified.
